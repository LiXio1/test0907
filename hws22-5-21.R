...

{
  ...
  source('E:/a/CUG/R/librarys&funcs.R')  # source(paste0(tfn, '???拾?.R'))
  ChinaMap_.25 <- readRDS(paste0(tfn, 'ChinaMap2_.25.RDS'))
  # ?????泄??????透??悖?C:/Users/xioli/Documents/Untitled5.R
...
## 统?坪???  Statistical time features
{
  Scalar_Statistics <- function(Hwsm) {  # ID time lon lat lonxy latxy
    ...
    if (sum(c('ID', 'time', 'lon', 
      ...
      # # st_length(line_st) # 圆  Mydistance  ????圆 
      # geoDist(lat1, lon1, lat2, lon2, NAOK = TRUE, DUP = TRUE) ???? ??Mydistance 一??   geoXY?木?????geoDist????
      ...
      # ??为?婕??转?堑?平???????远?一??转?纱???????   geoXY ??lat0纬圈????lonX????
      ...
      for (id in unique(Output$ID[which(Output$duation >= 2)])) { # unique(Output$ID[which(Output$duation == 10)])
        ...
        # # ??转??  ?嵌取?u v??geoXY????????????系????  ??小???睿好???叩???转
        # # ! # ????转??止??  v ???鼐??叩??俣?  u????转???????辖?为原?恪???惆????展??平???慕?
        # XY <- geoXY(points2$lat, points2$lon, unit = 1) # 转???芽?????为????
        # points2$lonX <- XY[, 1]
        # points2$latY <- XY[, 2] # ??证??Mydistance(points2[c('time', 'lon', 'lat')])
        ...
        temp <- data.table(
          ...
          # azimuth = angle3(c(p2$lonX[1] - p1$lonX[1], p2$latY[2] - p1$latY[2]))# ??位??
        )
        # s_uv ????平???系??俣?
...
hws0 <- ddply(hws0, c('ID', 'time'), summarise, S = sum(area), # ????每1??km2 HI
              Clon = sum(Percentile(HI*area)/sum(Percentile(HI*area))*lon), # ????占??(???????叩牡慵??)
...
#####
## 路?????莸募?????????时?????校?Scalar_Statistics(hws)  # ID time lon lat lonxy latxy
#####
